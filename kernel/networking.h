...
#define NETWORK_CONTROLLER_CLASSCODE 0x2
#define ETHERNET_CONTROLLER_SUBCLASS 0x0
...
#define CMD_LEGACY                      (1 << 5)    // Set Legacy Packet
...
typedef struct __attribute__ ((packed)) receiver_desc {
...
typedef struct __attribute__ ((packed)) transmitter_desc {
    volatile uint32_t address[2];
    volatile uint16_t length;
...
typedef struct ethernet_card {
    uint32_t bar0_type;
    uint32_t mem_base_addr;
    uint32_t irq_num;
    int has_eeprom;
    uint8_t mac[6];
    volatile receiver_desc_t * r_desc_ptrs[E1000_NUM_RX_DESC];
    int r_cur;
    volatile transmitter_desc_t * t_desc_ptrs[E1000_NUM_TX_DESC];
    int t_cur;
} ethernet_card_t;
